...
void setup() {
  ...
  myServo.write(140); // Initial position
...
void controlDevices() {
  ...
  if (systemActive && !lastState) {
    ...
    lastState = false;
    Serial.println("Ventilation system DEACTIVATED.");
  }
}
...
void sendSensorData() {
  ...
  payload += "\"DeviceID\":2000,";
  payload += "\"ActuatorID\":2009,";
  ...
  if (responseCode > 0) {
    ...
    if (action) {
      if (strstr(action, "Ventilation_ON") != nullptr) {
        Serial.println("Turning ON the ventilation system");
        systemActive = true;
        controlDevices();
      } else if (strstr(action, "Ventilation_OFF") != nullptr) {
        Serial.println("Turning OFF the ventilation system");
        systemActive = false;
        controlDevices();
      } else {
        Serial.print("No known ventilation command found in action: ");
        Serial.println(action);
      }
    } else {
      Serial.println("No 'action' field in response.");
    }
  } else {
